...
sig_matrix = as.matrix(cbind(sig1, sig2_r, sig2_tr, sig2_ma, sig2_qt, sig2_pk))
...
for ( i in c(1:dim(sig_matrix)[2])){
	...
	all_mean = c(log10(mean(sig1)+0.01), log10(mean(sig_tmp)+0.01))
	...
	plot_scatterplot_3parts(ref_all_s, tar_all_s, ref_cpk_s, tar_cpk_s, ref_cbg_s, tar_cbg_s, cpk_mean, cbg_mean, all_mean, -1.2, 2.1)
	dev.off()
}
