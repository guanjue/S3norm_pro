...
def NewtonRaphsonMethod(sig1_pk,sig1_bg, sig2_pk,sig2_bg, A,B, moment, converge_thresh, numIterations, total_length):
	...
	sig1_pk_mean = np.average(sig1_pk**moment, axis=0)
	sig1_bg_mean = np.average(sig1_bg**moment, axis=0)

	for i in range(0, numIterations):
		fb = sig1_bg_mean * np.average(sig2_pk**(moment*B), axis=0) - sig1_pk_mean * np.average(sig2_bg**(moment*B), axis=0)
		dfb = moment * sig1_bg_mean * np.average(np.log2(sig2_pk) * sig2_pk**(moment*B), axis=0) - moment * sig1_pk_mean * np.average(np.log2(sig2_bg) * sig2_bg**(moment*B), axis=0)

		### next step
		B = B - fb / dfb	
		A = sig1_bg_mean / np.average(sig2_bg**(moment*B), axis=0)
...
def pknorm(sig1_wg_raw, sig2_wg_raw, moment, B_init, fdr_thresh, sample_num, rank_lim, upperlim, lowerlim, script_folder, p_method):
	...
	#sig1_sig2_weight = np.repeat(1,sig1_s.shape[0])
	...
	sig1_cbg_mean = np.average(sig1_cbg, axis=0)
	print('check!check!')
	print(sig1_cbg_mean)
	print(np.mean(sig1_cbg))
	sig2_cbg_mean = np.average(sig2_cbg, axis=0)
	sig1_cpk_mean = np.average(sig1_cpk, axis=0)
	sig2_cpk_mean = np.average(sig2_cpk, axis=0)
	print(sig2_cbg_mean)
	...
	AB = NewtonRaphsonMethod(sig1_cpk+small_num,sig1_cbg+small_num, sig2_cpk+small_num,sig2_cbg+small_num, 1.0, 2.0, moment, 1e-5, 500, all_len)
