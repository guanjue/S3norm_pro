...
for (m in methods){
	...
	cor_0 = apply(cbind(rna_tpm, d_raw)[used_id_rna_tpm,], 1, function(x) cor((x[1:12]+small_num),(x[13:24]+small_num), method=cor_method))
	...
	cor_0_shuffle = apply(cbind(rna_tpm, d_raw_shuffle)[used_id_rna_tpm,], 1, function(x) cor((x[1:12]+small_num),(x[13:24]+small_num), method=cor_method))
#	d_raw_bg = read.table(paste('tss_atac.pcsorted.', m, '.1000kb.txt', sep=''), header=F)
#	cor_0_bg = apply(cbind(rna_tpm, d_raw_bg)[used_id_rna_tpm,], 1, function(x) cor((x[1:12]+small_num),(x[13:24]+small_num), method=cor_method))
...
barplot(kl_dist_vec[,c(1,2,3,4,5)], main="KL-dist", xlab="Methods", names.arg=c('NBP', 'NBP-TSnorm', 'NBP-QTnorm', 'NBP-MAnorm', 'NBP-PKnorm'))
#box()
dev.off()

pdf('cor_0_paired_t_statistic_vec.pdf', width=20, height=9)
barplot(paired_t_statistic_vec[,c(1,2,3,4,5)], main="KL-dist", xlab="Methods", names.arg=c('NBP', 'NBP-TSnorm', 'NBP-QTnorm', 'NBP-MAnorm', 'NBP-PKnorm'))
#box()
dev.off()
...
add=1
png('check.png')
d_raw = read.table(paste('tss_atac.pcsorted.', 'pknorm', '.txt', sep=''), header=F)
plot(rna_tpm[,colnum], d_raw[,colnum+add], log='xy')
print(cor(log2(rna_tpm[,colnum]), log2(d_raw[,colnum])))
dev.off()

png('check2.png')
d_raw = read.table(paste('tss_atac.pcsorted.', 'raw', '.txt', sep=''), header=F)
plot(rna_tpm[,colnum], d_raw[,colnum+add], log='xy')
print(cor(log2(rna_tpm[,colnum]), log2(d_raw[,colnum])))
dev.off()
...
par(mfrow=c(2,3))
for (m in methods){
d_raw = read.table(paste('tss_atac.pcsorted.', m, '.txt', sep=''), header=F)
...
for (i in c(1:6)){
	...
	png(paste('tss_atac.pcsorted.difcor.', methods[i], '.png', sep=''))
...
for (i in c(1:11)){
	...
	m_c='qtnorm'
	cor_method = 'spearman'
	d_raw_pk = read.table(paste('tss_atac.pcsorted.', m_b, '.txt', sep=''), header=F)
	d_raw_qt = read.table(paste('tss_atac.pcsorted.', m_c, '.txt', sep=''), header=F)
...
for (m in methods){
	...
	for (i in c(1:10)){
		for (j in c((i+1):11)){
			...
			png(paste('tss_atac.pcsorted.dif.', m, toString(i), '_', toString(j), '.png', sep=''))
